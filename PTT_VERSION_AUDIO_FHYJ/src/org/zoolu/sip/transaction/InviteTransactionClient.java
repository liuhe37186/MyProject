...
public class InviteTransactionClient extends TransactionClient {
	...
	//Add by zzhan 2013-04-28
	/** invite transaction timeout ("Timer B" in RFC 3261) */
	...
	void init(TransactionClientListener listener,
		...
		//Add by zzhan 2013-04-28
		invite_transaction_to = new Timer(SipStack.invite_transaction_timeout, "InviteTransaction",
				this);
	...
	public void onTimeout(Timer to) {
		try {
			...
			//Add by zzhan 2013-04-28
			if (to.equals(invite_transaction_to)) {
				printLog("Invite Transaction timeout expired", LogLevel.HIGH);
				retransmission_to.halt();
				transaction_to.halt();
				end_to.halt();
				sip_provider.removeSipProviderListener(transaction_id);
