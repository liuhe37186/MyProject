...
public class Call implements InviteDialogListener {
	...
	public void call(String callee) {
		...
		call(callee, null, null, null, null, false, false);
	}
	
	//Add by zzhan 2011-9-7
	/** Starts a new group call, inviting a remote user (<i>callee</i>) */
	public void groupcall(String callee) {
		call(callee, null, null, null, null, true, false);						// modified by mandrajg
	}

	/** Starts a new call, inviting a remote user (<i>callee</i>) */
	public void call(String callee, String sdp, String icsi) {		// modified by mandrajg
		call(callee, null, null, sdp, icsi, false, false);
	}
	
	//Add by zzhan 2011-9-7
	/** Starts a new group call, inviting a remote user (<i>callee</i>) */
	public void groupcall(String callee, String sdp, String icsi, boolean join_call) {		// modified by mandrajg
		call(callee, null, null, sdp, icsi, true, join_call);
	}

	//Modify by zzhan 2011-9-7
	/** Starts a new call, inviting a remote user (<i>callee</i>) */
	//public void call(String callee, String from, String contact, String sdp, String icsi)
	private void call(String callee, String from, String contact, String sdp, String icsi, boolean is_group_call, boolean join_call) {	// modified by mandrajg
		...
		if (local_sdp != null){
			if (!is_group_call)
				...
				dialog.groupinvite(callee, from, contact, local_sdp, icsi, join_call);	// modified by zzhan
		}
		else{
			if (!is_group_call)
				dialog.inviteWithoutOffer(callee, from, contact);
			else
				dialog.groupinviteWithoutOffer(callee, from, contact, join_call);		// modified by zzhan
		}
	}
