...
public class ExtendedCall extends Call implements ExtendedInviteDialogListener {
	...
	public void call(String r_user, String from, String contact, String sdp, String icsi){
		call(r_user, from, contact, sdp, icsi, false, false);
	}
	
	//Add by zzhan 2011-9-7
	/** Starts a new group call, inviting a remote user (<i>r_user</i>) */
	public void groupcall(String r_user, String from, String contact, String sdp, String icsi, boolean join_call){
		call(r_user, from, contact, sdp, icsi, true, join_call);
	}

	//Modify by zzhan 2011-9-7
	/** Starts a new call, inviting a remote user (<i>r_user</i>) */
	//public void call(String r_user, String from, String contact, String sdp, String icsi)
	private void call(String r_user, String from, String contact, String sdp, String icsi, boolean is_group_call, boolean join_call) {	// modified by mandrajg
		...
		if (local_sdp != null){
			if (!is_group_call)
				...
				dialog.groupinvite(r_user, from, contact, local_sdp, icsi, join_call);		// modified by zzhan
		}
		else{
			if (!is_group_call)
				dialog.inviteWithoutOffer(r_user, from, contact);
			else
				dialog.groupinviteWithoutOffer(r_user, from, contact, join_call);		// modified by zzhan
		}
	...
	//add by oumogang 2013-04-26  
		//创建扩展会话
		public void antaCall4(String r_user, String numbers ,String from, String contact, String sdp, String icsi,boolean isGroupBroadcast) {	// modified by mandrajg
			printLog("calling " + r_user, LogLevel.MEDIUM);
			if (username != null)
				dialog = new ExtendedInviteDialog(sip_provider, username, realm,
						passwd, this);
...
//				else
//					dialog.groupinvite(r_user, from, contact, local_sdp, icsi, join_call);		// modified by zzhan
				dialog.antaInvite(r_user, numbers,from, contact, local_sdp, icsi, isGroupBroadcast);
			}
			else{
//				if (!is_group_call)
//					dialog.inviteWithoutOffer(r_user, from, contact);
//				else
//					dialog.groupinviteWithoutOffer(r_user, from, contact, join_call);		// modified by zzhan
				dialog.antaInviteWithoutOffer(r_user, numbers,from, contact, isGroupBroadcast);
			}
