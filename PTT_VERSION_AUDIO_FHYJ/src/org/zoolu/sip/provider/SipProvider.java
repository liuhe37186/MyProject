...
import com.zed3.utils.LogControl;
...
public class SipProvider implements Configurable, TransportListener,
	...
	Vector<SipProviderListener> invite_listeners = null;
	...
	private void init(String viaddr, int port, String[] protocols, String ifaddr) {
		...
		//Added by zzhan 2011-11-03
		invite_listeners = new Vector<SipProviderListener>();
	...
	//Add by zzhan 2011-11-03
	public void addSipProviderInviteListener(SipProviderListener listener){
		invite_listeners.add(listener);
	}
	
	public void removeSipProviderInviteListener(SipProviderListener listener){
		invite_listeners.remove(listener);
	}
	...
	private ConnectionIdentifier sendMessage(Message msg, String proto,
		...
		if (transport_udp && proto.equals(PROTO_UDP)) { // UDP
			...
			if (connections == null || !connections.containsKey(conn_id)) { // modified
				...
				addConnection(conn);
				if (!msg.isRegister())
					Receiver.engine(Receiver.mContext).register(true); // modified
			} else {
	...
	protected void processReceivedMessage(Message msg) {
		try { // logs
			...
			if(msg.toString().startsWith("CCC") || msg.toString().length() == 2){//\r\n也要输出
				processHeartBeatMessage(msg.toString());
				return;
			}
			...
			//Process INFO request, before transaction process
			if (msg.isRequest() 
					&& msg.isRequest(SipMethods.INFO)
					&& listeners.containsKey(new TransactionIdentifier(SipMethods.INFO))){
				((SipProviderListener) listeners.get(new TransactionIdentifier(SipMethods.INFO))).onReceivedMessage(
						this, msg);
				return;
			}
			...
			if (msg.isRequest() && msg.isRequest(SipMethods.INVITE)){
				for (int i = 0; i < invite_listeners.size(); i++){
					InviteTransactionServer l = (InviteTransactionServer)invite_listeners.get(i);
					if (l.isWaiting()){
						l.onReceivedMessage(this, msg);
						return;
					}
				}
				
				//No idle lines, respond 486 busy
...
//		MyLog.i("sipstack", str); //modify by hu 2014-1-3  don't need print
	}
...
//		}
		...
		if (msg != null)
			msgx += ": \r\n" + msg.toString() + "\r\n";
		if(LogControl.siplogprint){
			MyLog.e("siptrace", msgx);////modify by hu 2014-1-3  don't need print  加控制
			LogUtil.makeLog("SipProvider", "printMessageLog()"+msgx);
		}
		
	}
	private IHeartBeatListener heartBeatListener = null;
	public void setHeartBeatListner(IHeartBeatListener listener){
		if(listener == null) return;
		this.heartBeatListener = listener;
	}
	/**
	 * send heartbeat msg  add by hu 2014/2/12
	 * */
	public void sendMessage(String heartbeatPacket){
		try {
			if(outbound_proxy != null){
			String dest_addr = outbound_proxy.getAddress().toString();
			int dest_port = outbound_proxy.getPort();
			if(udp != null){
				udp.sendMessage(heartbeatPacket, dest_addr, dest_port);
			}else{
			}
			}
		} catch (IOException e) {
	 ...
	 * */
	private void processHeartBeatMessage(String msg) {
		if(heartBeatListener != null){
			heartBeatListener.onReceiveHeatBeatMsg(msg);
		}
	}
}
