...
public class InviteDialog extends Dialog implements TransactionClientListener,
	...
	public void invite(String callee, String caller, String contact,
			String session_descriptor, String icsi) {
		invite(callee, caller, contact, session_descriptor, icsi, false, false);
	}
	
	//Add by zzhan 2011-9-7
	public void groupinvite(String callee, String caller, String contact,
			String session_descriptor, String icsi, boolean join_call) {
			invite(callee, caller, contact, session_descriptor, icsi, true, join_call);
		}
	
	
	//Add by oumogang 2013-09-02
	public void antaInvite(String callee,String numbers,String caller, String contact,
			String session_descriptor, String icsi, boolean isGroupBroadcast) {
		invite(callee, numbers,caller, contact, session_descriptor, icsi,isGroupBroadcast);
	}
	//Modify by zzhan 2011-9-7
	/**
	 ...
	 *            
	 */
	//void invite(String callee, String caller, String contact,
			//String session_descriptor, String icsi) {
	private void invite(String callee, String caller, String contact,
			String session_descriptor, String icsi, boolean is_group_call, boolean join_call) {							// modified by mandrajg
		...
		//Add by zzhan 2011-9-7
		//Add header "Ptt-Extension: 3ghandset create"
		if (is_group_call){
			if (!join_call)
				invite.setHeader(new Header(SipHeaders.Ptt_Extension, "3ghandset create"));
			else
				invite.setHeader(new Header(SipHeaders.Ptt_Extension, "3ghandset rejoin"));
		}
		//guojunfeng add 20140514
		if(DeviceInfo.isEmergency){
			invite.setHeader(new Header(SipHeaders.Ptt_emergency_call,com.zed3.sipua.ui.Settings.getUserName()));
	...
	//add by oumoang 2013-09-02
		private void invite(String callee, String numbers,String caller, String contact,
				String session_descriptor, String icsi, boolean isGroupBroadcast) {							// modified by mandrajg
			printLog("inside invite(callee,caller,contact,sdp)", LogLevel.MEDIUM);
			if (!statusIs(D_INIT))
				return;
			// else
...
//					invite.setHeader(new Header(SipHeaders.Ptt_Extension, "3ghandset rejoin"));
//			}
			//Anta-Extension: conference number1 number2
			//Anta-Extension: groupbroadcast number1 number2
			if (!isGroupBroadcast)
				invite.setHeader(new Header(SipHeaders.Anta_Extension, "conference "+numbers));
			else
				invite.setHeader(new Header(SipHeaders.Anta_Extension, "groupbroadcast "+numbers));
	...
	public void inviteWithoutOffer(String callee, String caller, String contact) {
		invite_offer = false;
		inviteWithoutOffer(callee, caller, contact, false, false);
	}
	
	//Add by zzhan 2011-9-7
	public void groupinviteWithoutOffer(String callee, String caller, String contact, boolean join_call) {
		invite_offer = false;
		inviteWithoutOffer(callee, caller, contact, true, join_call);
	}
	 ...
	 */
	//public void inviteWithoutOffer(String callee, String caller, String contact)
	private void inviteWithoutOffer(String callee, String caller, String contact, boolean is_group_call, boolean join_call) {
		invite_offer = false;
		if (!is_group_call)
			invite(callee, caller, contact, null, null);		// modified by mandrajg
		else
			groupinvite(callee, caller, contact, null, null, join_call);	// modified by zzhan
	}

	//Add by oumogang 2013-04-26
		public void antaInviteWithoutOffer(String callee,String numbers,String caller, String contact, boolean isGroupBroadcast) {
			invite_offer = false;
//			if (!is_group_call)
//				invite(callee, caller, contact, null, null);		// modified by mandrajg
//			else
//				groupinvite(callee, caller, contact, null, null, join_call);	// modified by zzhan
			...
			antaInvite(callee, numbers, caller, contact, null, null, isGroupBroadcast);
		}
	...
	public void refuse() {
		...
		// refuse(603,"Decline");
		refuse(486, SipResponses.reasonOf(486));//½«403¸Ä³É486
	}
	 ...
	 * TransactionClient
	 */
	private void bye(boolean is_group_call) {
		...
		if (statusIs(D_CALL)) {
			...
			if (is_group_call)
				bye.setHeader(new Header(SipHeaders.Ptt_Extension, "3ghandset"));
