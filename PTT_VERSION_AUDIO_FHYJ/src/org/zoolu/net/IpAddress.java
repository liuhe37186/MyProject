			...
			for (Enumeration<NetworkInterface> en = NetworkInterface.getNetworkInterfaces(); en.hasMoreElements();) {
				...
				for (Enumeration<InetAddress> enumIpAddr = intf.getInetAddresses(); enumIpAddr.hasMoreElements();) {
					...
					if (!inetAddress.isLoopbackAddress() && InetAddressUtils.isIPv4Address(inetAddress.getHostAddress())) { 
						...
						if (!defaultSharedPreferences.getBoolean(Settings.PREF_STUN, Settings.DEFAULT_STUN)) {
...
//								String StunServer = PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings.PREF_STUN_SERVER, Settings.DEFAULT_STUN_SERVER);
								String StunServer = defaultSharedPreferences.getString(Settings.PREF_STUN_SERVER, Settings.DEFAULT_STUN_SERVER);
