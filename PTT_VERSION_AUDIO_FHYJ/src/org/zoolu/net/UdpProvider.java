	...
	public void send(UdpPacket packet) throws IOException {
		if (!stop){
			...
			if(packet.getLength()+42>60){
				flow_send =  packet.getLength()+42;
			}else{
				flow_send = 60;
			}
			FlowStatistics.Sip_Send_Data = FlowStatistics.Sip_Send_Data+flow_send;
		}
...
//			socket.setSoTimeout(socket_timeout); modified
			...
			while (!stop) {
				try {
					...
					FlowStatistics.Sip_Receive_Data=flow+FlowStatistics.Sip_Receive_Data;
