...
public abstract class CodecInfo {
	...
	public static List<String> supportCodecType = new ArrayList<String>();
	static {
		...
		for (int codecIndex = 0, codecCount = MediaCodecList.getCodecCount(); codecIndex < codecCount; codecIndex++) {
			MediaCodecInfo codecInfo = MediaCodecList
					.getCodecInfoAt(codecIndex);
			CodecInfo ci = CodecInfo.getCodecInfo(codecInfo);
//			if(supportCodecType == null ||supportCodecType.size() < 1){
				supportCodecType = CodecInfo.getSupportCodecType(codecInfo);
//			}
			if (ci != null) {
				codecs.add(ci);
				// Log.e("htest", ""+ci.getName());
				// if(bannedYuvCodecs.contains(ci.getName())){
				// }
				// codecs.add(ci);
	 ...
	 * 
	 * @param codecInfo
	 *            the codec info object to encapsulate.
	 * @param mediaType
	 ...
	 * @param isEncoder
	 *            <tt>true</tt> if encoder should be returned or <tt>false</tt>
	 *            for decoder.
	...
	public static CodecInfo getCodecForType(String mimeType, boolean isEncoder) {
		for (CodecInfo codec : codecs) {
			if (!codec.isBanned() && codec.mediaType.equals(mimeType)
	...
	public static List<String> getSupportCodecType(MediaCodecInfo codecInfo) {
		String[] types = codecInfo.getSupportedTypes();
		for (String type : types) {
			if (!TextUtils.isEmpty(type) && type.contains("video")) {
				if (type.contains("vp8") && !supportCodecType.contains("vp8")) {
					supportCodecType.add("vp8");
				} else if (type.contains("vp9")
						&& !supportCodecType.contains("vp9")) {
					supportCodecType.add("vp9");
				} else if (type.contains("avc")
						&& !supportCodecType.contains("h264")) {
					supportCodecType.add("h264");
				} else if (type.contains("hevc")
						&& !supportCodecType.contains("h265")) {
					supportCodecType.add("h265");
				} else if (type.contains("mp4v-es")
						&& !supportCodecType.contains("MPEG4")) {
					supportCodecType.add("MPEG4");
				} else if (type.contains("3gpp")
						&& !supportCodecType.contains("h263")) {
					supportCodecType.add("h263");
				}

			}
		}
		return supportCodecType;
	}

	public static CodecInfo getCodecInfo(MediaCodecInfo codecInfo) {
		if (supportCodecType == null) {
			supportCodecType = new ArrayList<String>();
		}
		String[] types = codecInfo.getSupportedTypes();
		for (String type : types) {
			try {
				if (type.equals(MEDIA_CODEC_TYPE_H263))
					return  null;//new H263CodecInfo(codecInfo);
				else if (type.equals(MEDIA_CODEC_TYPE_H264))
					return  new H264CodecInfo(codecInfo);
				else if (type.equals(MEDIA_CODEC_TYPE_VP8))
					return null;//new VP8CodecInfo(codecInfo);
				else if(type.equals(MEDIA_CODEC_TYPE_H265)){
					return  null;//new H265CodecInfo(codecInfo);
				}
	...
	public String getLibjitsiEncoding() {
		if (mediaType.equals(MEDIA_CODEC_TYPE_H263)) {
			return /* Constants.H263P */"h263";
		} else if (mediaType.equals(MEDIA_CODEC_TYPE_H264)) {
			return /* Constants.H264 */"h264";
		} else if (mediaType.equals(MEDIA_CODEC_TYPE_VP8)) {
			return /* Constants.VP8 */"vp8";
		} else if (mediaType.equals(MEDIA_CODEC_TYPE_H265)) {
			return /* Constants.H264 */"h265";
		}else {
			return mediaType;
		}
	}
	...
	static class H264CodecInfo extends CodecInfo {
		...
		private final CodecInfo.Level[] LEVELS = new CodecInfo.Level[] {
				...
				new Level("Level1", 0x01), new Level("Level1b", 0x02),
				new Level("Level11", 0x04), new Level("Level12", 0x08),
				new Level("Level13", 0x10), new Level("Level2", 0x20),
				new Level("Level21", 0x40), new Level("Level22", 0x80),
				new Level("Level3", 0x100), new Level("Level31", 0x200),
				new Level("Level32", 0x400), new Level("Level4", 0x800),
				new Level("Level41", 0x1000), new Level("Level42", 0x2000),
				new Level("Level5", 0x4000), new Level("Level51", 0x8000) };
	...
	static class H265CodecInfo extends CodecInfo {
		...
		public H265CodecInfo(MediaCodecInfo codecInfo) {
			super(codecInfo, MEDIA_CODEC_TYPE_H265);
		}
	...
	static class H263CodecInfo extends CodecInfo {
		private final CodecInfo.Profile[] PROFILES = new CodecInfo.Profile[] {
				...
				new Profile("Baseline", 0x01), new Profile("H320Coding", 0x02),
				new Profile("BackwardCompatible", 0x04),
				new Profile("ISWV2", 0x08), new Profile("ISWV3", 0x10),
				new Profile("HighCompression", 0x20),
				new Profile("Internet", 0x40), new Profile("Interlace", 0x80),
				new Profile("HighLatency", 0x100) };

		private final CodecInfo.Level[] LEVELS = new CodecInfo.Level[] {
				// from OMX_VIDEO_H263LEVELTYPE
				new Level("Level10", 0x01), new Level("Level20", 0x02),
				new Level("Level30", 0x04), new Level("Level40", 0x08),
				new Level("Level45", 0x10), new Level("Level50", 0x20),
				new Level("Level60", 0x40), new Level("Level70", 0x80) };
	...
	static class VP8CodecInfo extends CodecInfo {
		...
		private final Level[] LEVELS = new Level[] {
				...
				new Level("Version0", 0x01), new Level("Version1", 0x02),
				new Level("Version2", 0x04), new Level("Version3", 0x08) };
