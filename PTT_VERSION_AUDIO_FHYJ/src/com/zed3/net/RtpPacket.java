...
public class RtpPacket {
	...
	public void setPackedTime(long packedTime) {
		this.packedTime = packedTime;
	}
	//Added by zzhan 2011-9-20
	...
	/** Sets the cmr byte */
	public void setCmr(byte cmrValue) {
		
		if (packet_len >= 13){
			packet[12] = (byte) (cmrValue & 0x7F);
			packet[12] = (byte)((packet[12]<<4) & 0xf0);
		}
	}
	...
	/** Creates a new RTP packet */
	public RtpPacket(byte[] buffer, int packet_length , int i) {
		packet = buffer;
		packet_len = packet_length;
		if (packet_len < 12)
			packet_len = 12;
	...
	static final long ssrc = Random.nextLong();
	...
	private static boolean getBit(byte b, int bit) {
		//return (b >> bit) == 1;
		return (b & 0x80) == 128;//修改了移位 eidt by zzhan 20130225
	}
