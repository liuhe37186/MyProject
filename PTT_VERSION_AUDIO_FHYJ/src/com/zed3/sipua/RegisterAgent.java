...
public class RegisterAgent implements TransactionClientListener, SubscriberDialogListener {
	...
	public boolean register() {
		//use the SipStack.default_expires to apply new expire_time. modify by mou 2014-11-03
		return register(/*expire_time*/SipStack.default_expires);
	}
	...
	public boolean register(int expire_time) {
		...
		t = new TransactionClient(sip_provider, req, this, 30000);//modify by hu 30000
		t.request();
		...
		if (transaction.getTransactionMethod().equals(SipMethods.REGISTER)) {
			...
			if (CurrentState == REGISTERING)
			{
				...
				if (listener != null)
				{
					/*
					 * Add by zzhan 2011-10-25
					 * Process ptt groups, format As follows:
					 * 
					 * Content-Type: text/3ghandset
					 * Content-Length: xxx
					...
					if (cth != null){
						if (cth.getValue().equalsIgnoreCase("text/3ghandset")){
							result = resp.getBody();
							// 用户名，协议升级携带用户名信息
							// add by liangzhang 2014-09-10 获取用户名并保存
							String displayName = resp.getFromHeader()
									.getNameAddress().getDisplayName();
							// 获取用户号码，当获取不到用户名时显示用户号码
							// add by liangzhang 2014-10-23
							String userName = resp.getFromHeader()
...
//					}
	...
	public void onTransFailureResponse(TransactionClient transaction,
		...
		if (transaction.getTransactionMethod().equals(SipMethods.REGISTER)) {
			...
			if (!processAuthenticationResponse(transaction, resp, code)) {
				...
				//previous state
				Header hd = resp.getHeader("Ptt-Extension");
	...
	public void onTransTimeout(TransactionClient transaction) {
		...
		if (transaction.getTransactionMethod().equals(SipMethods.REGISTER)) {
			...
			//Since the transactions are atomic, we rollback to the 
			//previous state
			//如果信号不够好，会在通话过程中出现超时现象，可以强制转成绿点，不让调用halt方法，继续发送视频包   edit by hdf 20121017
			if (Receiver.call_state == UserAgent.UA_STATE_INCALL ) {//Receiver.call_state !=0
				CurrentState = REGISTERED;
				if (listener != null)
				{
					listener.onUaRegistrationSuccess(this, target, contact, "");
					Receiver.reRegister(user_profile.expires);
