      ...
      int force_speech )
{
   ...
   if (noHoming){
      ...
      switch ( mode ) {
         case MR122:
        	LOGI("noHoming == 0 && mode == MR122");
            homing = dhf_MR122;
            homing_size = 18;
        	LOGI("noHoming = %d && homing_size == %d",dhf_MR122,18);
            break;

         case MR102:
        	LOGI("noHoming == 0 && mode == MR102");
            homing = dhf_MR102;
            homing_size = 12;
        	LOGI("noHoming = %d && homing_size == %d",dhf_MR102,12);
            break;

         case MR795:
        	LOGI("noHoming == 0 && mode == MR795");
            homing = dhf_MR795;
            homing_size = 8;
        	LOGI("noHoming = %d && homing_size == %d",dhf_MR795,8);
            break;

         case MR74:
        	LOGI("noHoming == 0 && mode == MR74");
            homing = dhf_MR74;
            homing_size = 7;
        	LOGI("noHoming = %d && homing_size == %d",dhf_MR74,7);
            break;

         case MR67:
        	LOGI("noHoming == 0 && mode == MR67");
            homing = dhf_MR67;
            homing_size = 7;
        	LOGI("noHoming = %d && homing_size == %d",dhf_MR67,7);
            break;

         case MR59:
        	LOGI("noHoming == 0 && mode == MR59");
            homing = dhf_MR59;
            homing_size = 7;
        	LOGI("noHoming = %d && homing_size == %d",dhf_MR59,7);
            break;

         case MR515:
        	LOGI("noHoming == 0 && mode == MR515");
            homing = dhf_MR515;
            homing_size = 7;
        	LOGI("noHoming = %d && homing_size == %d",dhf_MR515,7);
            break;

         case MR475:
        	LOGI("noHoming == 0 && mode == MR475");
            homing = dhf_MR475;
            homing_size = 7;
        	LOGI("noHoming = %d && homing_size == %d",dhf_MR475,7);
            break;
   ...
   if ( used_mode == MRDTX ) {
      ...
      if ( s->prev_ft == TX_SPEECH_GOOD ) {
         ...
         /* TX_SID_UPDATE or TX_NO_DATA */
			if ((s->sid_handover_debt > 0) && (s->sid_update_counter > 2)) {
				...
				LOGI("used_mode == MRDTX  3");
				if (s->sid_update_counter == 0) {
					LOGI("used_mode == MRDTX  4");
					txFrameType = TX_SID_UPDATE;
					s->sid_update_counter = 8;
				} else {
					LOGI("used_mode == MRDTX  5");
      ...
      LOGI("used_mode != MRDTX  txFrameType = TX_SPEECH_GOOD");
   }
   s->prev_ft = txFrameType;

   if ( noHoming == 0 ) {
	  LOGI("noHoming = 0,Speech_Encode_Frame_reset,encoderState = %d ,dtx = %d",s->encoderState,s->dtx);
